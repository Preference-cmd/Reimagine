...
#[test]
fn model_dependent_nodes_declare_resource_requirements() {
    let catalog = BuiltinNodeCatalog::v1();

    // checkpoint_loader: CheckpointBundle, ~6 GB
    let checkpoint = catalog
        .get(&NodeTypeId::new(BUILTIN_CHECKPOINT_LOADER))
        .unwrap();
    let reqs = checkpoint
...
#[test]
fn pure_and_io_nodes_have_no_resource_requirements() {
    let catalog = BuiltinNodeCatalog::v1();
    ...
    for type_id in [
        ...
        let node = catalog
            .get(&NodeTypeId::new(type_id))
            .unwrap_or_else(|| panic!("node {type_id} exists"));
        assert!(
            node.resource_requirements().is_none(),
            "{type_id} should have no resource requirements"
        );
    }
}
