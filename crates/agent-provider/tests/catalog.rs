//! Wiremock-driven tests for `ModelCatalog`: load/fallback semantics,
//! override replacement, refresh persistence, and corrupt-file
//! tolerance.

use reimagine_agent_provider::{CatalogError, ModelCatalog};
...
async fn refresh_all(catalog: &mut ModelCatalog, paths: &AppPaths) -> Result<(), CatalogError> {
...
async fn load_with_missing_cache_returns_empty_catalog() {
    ...
    let catalog = ModelCatalog::load(&paths).await.expect("load ok");
...
async fn load_with_corrupt_cache_returns_error() {
    ...
    let error = ModelCatalog::load(&paths)
        .await
...
async fn refresh_fetches_persists_and_replaces_catalog() {
    ...
    let persisted = ModelCatalog::load(&paths)
        .await
...
async fn refresh_failure_without_cache_falls_back_to_snapshot() {
    ...
    assert!(matches!(error, CatalogError::Fetch(_)));
    assert_eq!(
        catalog.providers(),
...
async fn override_replaces_provider_entry_wholesale() {
    ...
    let loaded = ModelCatalog::load(&paths)
        .await
...
async fn override_missing_file_is_noop() {
    ...
    let loaded = ModelCatalog::load(&paths).await.expect("load ok");
