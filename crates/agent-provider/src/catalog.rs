...
//!
//! `ModelCatalog` holds a registry of `provider -> models` entries
...
//! left untouched. Overrides are applied by [`ModelCatalog::load`] and
//! re-applied after a successful [`ModelCatalog::refresh`].
//!
//! # Loading and fallback
//!
//! - [`ModelCatalog::load`]: reads the cache file; a missing cache file
//!   yields an empty catalog, a corrupt one returns
//!   [`CatalogError::Parse`].
//! - [`ModelCatalog::refresh`]: fetches `https://models.dev/api.json`
...
use reimagine_agent::{ModelCost, ModelInfo, ModelName, ProviderName};
...
/// written deterministically. [`ModelCatalog::providers`] returns ids
...
impl ModelCatalog {
    ...
    pub async fn load(paths: &AppPaths) -> Result<Self, CatalogError> {
        ...
        let mut catalog = match tokio::fs::read_to_string(&cache_path).await {
            ...
            Err(error) => return Err(CatalogError::read(error.to_string())),
        };
    ...
    pub async fn refresh_with_http_client(
        ...
        let json = serde_json::to_string_pretty(&fetched)
            .map_err(|error| CatalogError::write(error.to_string()))?;
    ...
    async fn apply_override(&mut self, config_dir: &Path) -> Result<(), CatalogError> {
...
async fn fetch_models_dev(
    ...
    if !status.is_success() {
        return Err(CatalogError::fetch(format!("HTTP {status}")));
    }
