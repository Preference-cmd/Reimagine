...
pub enum AgentEvent {
    ...
    /// A streaming reasoning / thinking delta from the provider.
    /// Emitted before `ContentDelta` for models that reason. Display-only:
    /// the runtime never replays reasoning into message history.
    ReasoningDelta {
        session_id: AgentSessionId,
        text: String,
    },
