...
fn expand_inner(attr: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    ...
    let mode_tokens = args
        ...
        .map(|mode| match mode.as_str() {
            "agent" => quote!(::reimagine_agent_harness::AgentMode::Agent),
            "build" => quote!(::reimagine_agent_harness::AgentMode::Build),
            _ => unreachable!("validated mode"),
        })
        .collect::<Vec<_>>();
    let risk_token = match args.risk.as_deref().unwrap_or("read") {
        "read" => quote!(::reimagine_agent_harness::ToolRiskLevel::Read),
        "editor" => quote!(::reimagine_agent_harness::ToolRiskLevel::Editor),
        "external" => quote!(::reimagine_agent_harness::ToolRiskLevel::External),
    ...
    Ok(quote! {
        ...
        impl ::reimagine_agent_harness::AgentTool for #wrapper_ident {
            fn spec(&self) -> ::reimagine_agent_harness::ToolSpec {
                ...
                ::reimagine_agent_harness::ToolSpec::new(
                    ::reimagine_agent_harness::ToolName::new(#name),
                    #description,
                    [#(#mode_tokens),*],
                    ::reimagine_agent_harness::ToolPermission::new(#permission),
                    #risk_token,
                )
            ...
            async fn invoke(
                &self,
                ctx: &::reimagine_agent_harness::ToolContext,
                input: ::reimagine_agent_harness::ToolInput,
            ) -> ::reimagine_agent_harness::ToolResult {
                let typed_input: #input_ty = ::serde_json::from_value(input).map_err(|err| {
                    ::reimagine_agent_harness::ToolError::new(
                        ::reimagine_agent_harness::ToolErrorCode::InvalidInput,
                        format!("failed to deserialize tool input: {err}"),
                    )
                    .with_tool(::reimagine_agent_harness::ToolName::new(#name))
                })?;
                let typed_output = #fn_name(ctx.clone(), typed_input).await?;
                ::serde_json::to_value(typed_output).map_err(|err| {
                    ::reimagine_agent_harness::ToolError::new(
                        ::reimagine_agent_harness::ToolErrorCode::ExecutionFailed,
...
fn _assert_paths_compile() {
    let _: Type = parse_quote!(::reimagine_agent_harness::ToolContext);
}
